...
## 第六波（2026-07-09 実装）: リッチシーナリー（置き物・山・スタート）

- **裁定（toming・3弾全採用）**: 置き物パレット＋山＋スタート位置編集。全部テキスト生成の
  延長・エンジン改変ゼロ。
- **🚢 置き物（GOB）**: stock地上物108種は全てpreload済み＝NAMは常にリンク。GOBは7行の
  テキスト（POSの角度は32768=π）。**MPN/MPS省略で静止配置**＝空母も止まり、
  アレスティングワイヤ＋カタパルトは機能（fsgroundproperty.cpp:1258 の無パス空母分岐）。
  パレットは厳選18種（空母/高架滑走路1km/島基地/ビッグベン/城/橋/森/象/SAM/戦車…）。
- **⛰ 山（TER）**: コサイン減衰の TerrMesh を PCK インライン生成（NBL 16 16・CBEで
  標高グラデ緑→茶・最終行/列は簡略形・**PCK行数は厳密一致**）。グリッド原点は-X/-Z隅
  なので POS=中心-半径。接地はTERが自動優先（max）・クラッシュ判定は三角形法線を使う
  本物の斜面＝緩斜面は着陸可。
- **🛫 スタート**: .stpに複数Nブロック。低高度・速度0はスロットル0＋ギアダウンで生成。
  既定のSTART01（上空6000m南）は常に先頭＝?freeflightの既定は不変。
- **canvas配置UI**: モードツールバー（✏️/🚢/⛰/🛫）＋モード別パラメータ（向き/半径/高さ/
  高度/速度）。undoは配置順（型を跨いで1つ戻す）。レシピに全配置が乗る＝✏️再編集で
  置き物も山も戻る。
