#!/usr/bin/env bash
# Headline v2 smoke (M5): 2-browser host->joiner pack transfer over P2P.
# Serves dist/ + the test pack, runs a local signaling stub, drives two browsers.
#   scripts/build.sh && scripts/smoke-mp-pack.sh
set -euo pipefail
...
node "$ROOT/scripts/smoke-mp-pack.mjs" "http://localhost:$SERVE_PORT/index.html" "ws://localhost:$SIG_PORT/signal"
